...
public class DocumentStoreImpl implements DocumentStore
{
    ...
    public int putDocument(InputStream input, URI uri, DocumentFormat format) throws IOException
    {
        ...
        {
            d=btree.get(uri);
            if(d!=null) //something already there- now it is in memory from the get
            {
                if(this.docsInMemory.contains(d)) //if it was in memory before the call to put
                {
                ...
                else //it wasn't in memory before the call to get
                {
        ...
        else
        {
            ...
            else
            {
                ...
                if(this.docsInMemory.contains(d)) //d was in memory before this call of put
                {
                    wasInMemory=true;
                ...
                else //if d was on disk before this call to put
                {
                    this.updateStorageTrackersForAdd(document);
                }
                this.docsInMemory.add(document);
    ...
    //need to figure out how to pop the one i peeked off the stack;
    {
        if (commandStack.size()==0)
        {
            throw new IllegalStateException("stack is empty");
